...
namespace Sample
{
    public static class LessonTypes
    {
        ...
        {
            return Convert.ToDouble((((sagenes * 48) + (arshins * 16) + vershoks) * 4.445) / 100);
        }
        ...
        {
            return Math.Round(Convert.ToDouble(((deg * 3600) + (min * 60) + sec) * (Math.PI / 180 / 3600)), 5);
        }
        ...
        {
            double summa = initialMoney;
            if (percent == 0) return (double)initialMoney;
            for (int i = 0; i < years; i++) {
                summa += ((double)summa / 100) * percent;
            }
            return summa;
        }
